...
def train(bert, data_generator, training_config):
    ...
    training_config = {**training_config, **default_training_config}
    ...
    optimizer = t.optim.Adam(bert.parameters(), **training_config['optimizer_config'])
...
def save(bert, filename=None):
    
    if not filename:
        timestamp = datetime.datetime.now().strftime('%Y%m%d-%H%M')
        filename = '/home/ubuntu/mlab/days/w2d4/bert-classifier-%s.pt' % timestamp
