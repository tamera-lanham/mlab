...
def preprocess_batch(batch, gpu):
    labels, inputs = zip(*batch)
    labels = t.stack([label_to_tensor(label) for label in labels]).to(gpu)
...
def get_batches(rank, gpu, fake=True, batch_size=32, max_seq_len=512, n_fake_batches=10):
    ...
    if rank == 0:
        ...
        train_batches = [preprocess_batch(batch, gpu) for batch in train_batches]
        test_batches = [preprocess_batch(batch, gpu) for batch in test_batches]
...
def run(rank, size, all_groups): 
    ...
    for i in range(n_batches):
        ...
        train_labels = pass_labels_to_last(process_info, train_labels, all_groups['first_and_last'], (batch_size, 2))
        test_labels = pass_labels_to_last(process_info, test_labels, all_groups['first_and_last'], (batch_size, 2))
        
        if rank == size - 1:
            print(f'! {train_labels.shape}, {test_labels.shape}')
